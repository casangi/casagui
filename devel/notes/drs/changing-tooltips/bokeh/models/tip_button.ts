...
export class TipButtonView extends AbstractButtonView {
  ...
  protected predisplay: CallbackLike0<TipButtonView>
    ...
    const toggle = (visible: boolean) => {
        if ( visible && ! wait_function_set ) {
            ...
            setTimeout( ( ) => {
                ...
                if ( mouse_inside ) {
                    const {predisplay} = this.model
                    if ( predisplay != null ) predisplay!.execute( this )
                    console.log( '>>>>>---two-tooltips---->>', this.tooltip, this.tooltip.model )
...
export namespace TipButton {
  ...
  export type Props = AbstractButton.Props & {
    ...
    predisplay: p.Property<CallbackLike0<TipButtonView> | null>
...
export class TipButton extends AbstractButton {
  ...
  static {
    ...
    this.define<TipButton.Props>(({Ref,Any,Number}) => ({
      tooltip: [ Ref(Tooltip) ],
      predisplay: [ Any, null ],

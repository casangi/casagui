...
def main(argv):
    ...
    if len(argv) > 1:
        ...
        print(f"No ddi selected, using first ddi")
    ...
    # summary = pd df with columns [name, ddi, intent, field_id, field_name, start_frequency, end_frequency]
    ps_ddis = ps.summary()['ddi'].values.tolist()
    if ddi is not None and ddi not in ps_ddis:
        raise ValueError(f"Invalid ddi selection {ddi}. Please select from {set(ps_ddis)}")
    ...
    for key in ps.keys():
        ...
        if ddi is None:
            ddi = xds.ddi
            print(f"Raster plot for ddi {ddi} pol {xds.polarization.values[0]}")
        elif xds.ddi != ddi:
            continue
        ...
        #xds_list.append(xds)
        xds_partitions = partition_xds(xds)
        xds_list.extend(xds_partitions)

    # Concat xds and set amp, freq
    plot_xds = xr.concat(xds_list, dim='time')

    # Find color limits across all channels and pols
    ...
    amp_xda = plot_xds.VISIBILITY.sortby('time')
    flag_xda = plot_xds.FLAG.sortby('time')
